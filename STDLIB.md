...
| **8** | **Filesystem Project Identity** | `github.com/google/uuid`, `github.com/go-git/go-git` | `crypto/rand`, `os`, `filepath.EvalSymlinks` | Stores a random project identity marker inside each registered directory, so inode recycling cannot inherit a vault and cross-device moves preserve identity. | [`pkg/project/identity.go`](pkg/project/identity.go), [`identity_linux.go`](pkg/project/identity_linux.go), [`identity_darwin.go`](pkg/project/identity_darwin.go), [`identity_windows.go`](pkg/project/identity_windows.go) |
