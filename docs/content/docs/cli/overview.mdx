...
| `mf import` | `mf import [.env]` | Bulk-imports an existing `.env` file directly into vault | Path to `.env` |
| `mf rotate-password`| `mf rotate-password` | Re-encrypts all project vaults with a new master password | Interactive prompts |
| `mf run` | `mf run <cmd>` | Explicit alias for direct process execution | `--isolated` |
