...
- [x] **RFC 8018 PBKDF2 Key Derivation Engine**
  - Hand-rolled PBKDF2-HMAC-SHA256 (600,000 iterations) in `pkg/vault/kdf.go`.
- [x] **Authenticated Binary Vault Storage**
  - 15-byte authenticated binary header with AES-256-GCM AEAD encryption in `pkg/vault/vault.go`.
- [x] **In-Memory Process Injection & RAM Zeroization**
  - Direct execution via `os/exec.CommandContext` with volatile RAM environment overlay in `pkg/executor/process.go`.
- [x] **Filesystem Inode Project Identity**
  - Physical `(Dev, Ino)` resolution for Linux & macOS and Volume Serial Number for Windows in `pkg/project/`.
- [x] **Tamper-Evident Cryptographic Audit Trail**
  - Immutable SHA-256 hash-chained access log with verification in `pkg/audit/log.go`.
- [x] **Plaintext Leak Scanner**
  - Recursive AST/regex credential crawler in `pkg/scanner/scanner.go`.
- [x] **Two-Tier TUI Engine & Custom Widget Framework**
  - **Global Mode (`mayfly` / `mf`):** 2D interactive Project Directory Cards Grid with arrow navigation.
...
- [x] **All-in-One Global Installer (`install.sh` & `install.ps1`)**
  - Automatic installation, `--update`, and `--uninstall` provisioning both `mayfly` and `mf` commands in user `$PATH`.
...
- [ ] **Record 5-Minute Demo Video**
  - Screen capture with audio/captions showing:
    ...
    2. One-line installation via `./install.sh`.
